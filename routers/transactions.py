...
async def get_calendar_transaction_list(
    ...
    grouped_data = defaultdict(lambda: {"Итого": 0.0, "Лимит": 0.0, "Остаток по лимиту": 0.0})  # Default structure for each date
    ...
    for date, payment_type, total_value, limit_value in result:
        ...
        grouped_data[date_str]["Итого"] += float(total_value)  # Update total
        grouped_data[date_str]["Лимит"] += float(limit_value)  # Update limit
        grouped_data[date_str]["Остаток по лимиту"] = grouped_data[date_str]["Лимит"] - grouped_data[date_str]["Итого"]  # Update balance
